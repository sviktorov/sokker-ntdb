...
from arcades.models import CupDraw, Cup, CupTeams
...
DRAW_POT = {
    1: [1, 7, 8, 9, 15, 16, 25, 27, 35],
    2: [2, 4, 10, 12, 13, 17, 26, 29, 30],
    3: [6, 11, 18, 19, 20, 28, 32, 33, 34],
    4: [3, 5, 14, 21, 22, 23, 24, 31, 36],
}

def draw_cl(cup):
    print("Init Draw CL")
    teams = CupTeams.objects.filter(c_id=cup).delete()
    pots = CupDraw.objects.filter(c_id=cup).order_by("g_id")
    drawn_balls = []
    for team in pots:
        possible_pots = DRAW_POT[team.g_id]

        print(team.t_id)# This will show all attributes of the team object
        while True:
            pot = random.choice(possible_pots)
            if pot not in drawn_balls:
                drawn_balls.append(pot)
